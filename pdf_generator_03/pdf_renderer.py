...
class PDFRenderer:
    ...
    def create_monthly_report(self, country_code: str, country_name: str, 
        ...
        try:
            ...
            # Generate BLUF (without images for now to avoid complexity)
            ...
            # Add BLUF to PDF
            story.append(Paragraph(bluf_text, self.styles['bluf']))
            story.append(Spacer(1, 30))
            
            print("BLUF added to PDF successfully")
            
        except Exception as e:
            print(f"Error generating BLUF: {e}")
            import traceback
            traceback.print_exc()
            # Continue without BLUF if there's an error
